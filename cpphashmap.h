...
struct hash_map {
  ...
  struct iterator {
    ...
    size_t step(size_t i) {
      while (i < h->limit && (bitmap_get(h->bitmap, i) & occupied) != occupied)
        i++;
      return i;
    }
    iterator &operator++() {
  ...
  inline hash_map(hash_map &&o)
      : used(o.used), tombs(o.tombs), limit(o.limit), data(o.data),
        bitmap(o.bitmap) {
    o.data = nullptr;
    o.bitmap = nullptr;
  }
  ...
  enum bitmap_state { available = 0, occupied = 1, deleted = 2, recycled = 3 };
  ...
  bool operator==(const hash_map &o) const {
    for (auto i : const_cast<hash_map &>(*this)) {
      ...
      if (j == const_cast<hash_map &>(o).end())
        return false;
    ...
    for (auto i : const_cast<hash_map &>(o)) {
      ...
      if (j == const_cast<hash_map &>(*this).end())
        return false;
      if (i.second != j->second)
        return false;
    }
